...
import { GradientAreaChart, MetricBarChart, DonutChart, DensityHeatmap } from '../Charts';
import { RefreshCw, Download, Calendar, X, TrendingUp, Award, User, Clock } from 'lucide-react';
import { api } from '../../lib/api';
import { CHART_COLORS } from '../../constants';

export const OverviewView: React.FC = () => {
  const kpis = api.getOverviewKPIs();
  const revenueData = api.getRevenueTrends();
  const salesPerformance = api.getSalesRankings();
  const leadSources = api.getLeadSources();
  const heatmapData = api.getActivityHeatmap();
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        {/* Main Revenue Chart (Updated to Gradient Area) */}
          ...
          <div className="flex-1 w-full min-h-0">
            <GradientAreaChart 
              ...
              areas={[
                ...
                { key: 'previousRevenue', name: 'Previous Year', color: CHART_COLORS[1] } // Using transparent/secondary color
              ]}
              height={320} 
            />
          </div>
        ...
        {/* Top Salespeople (Updated to Modern Bar) */}
        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-card dark:shadow-none min-h-[420px] flex flex-col border border-transparent dark:border-slate-700">
          ...
          <div className="flex-1 w-full min-h-0">
            <MetricBarChart 
              ...
              xKey="name"
              bars={[{ key: 'value', name: 'Score', color: CHART_COLORS[5] }]}
              layout="vertical"
              height={320} 
      ...
      {/* Selected Performer Drill-down Panel (Unchanged structure, just repositioned for flow) */}
      {selectedPerformer && (
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-3 gap-4 max-w-lg">
                <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3 border border-gray-100 dark:border-slate-600">
                  ...
                  <div className="text-lg font-bold text-primary">{selectedPerformer.value}</div>
                </div>
                <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3 border border-gray-100 dark:border-slate-600">
                  <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-text-tertiary dark:text-gray-400 mb-1">
                ...
                <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3 border border-gray-100 dark:border-slate-600">
                  ...
                  <div className="text-lg font-bold text-text-primary dark:text-white">42</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-card dark:shadow-none border border-transparent dark:border-slate-700">
          ...
          <DonutChart data={leadSources} height={280} />
        </div>
        
        {/* Activity Heatmap (New Viz) */}
